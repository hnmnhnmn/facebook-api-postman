...
const UserDUT = require("../models/UserDUT");

//check user is login
router.get("/", verifyToken, async (req, res) => {
  try {
    const user = await UserDUT.findById(req.userId).select("-password");
    if (!user) {
      return res
        .status(400)
...
router.post("/signup", async (req, res) => {
  ...
  try {
    const userByUsername = await UserDUT.findOne({ username });
    const userByEmail = await UserDUT.findOne({ email });
    ...
    const newUser = new UserDUT({ username, email, password: hashedPassword });
    await newUser.save();
...
router.post("/signin", async (req, res) => {
  ...
  try {
    const user = await UserDUT.findOne({ email });
    if (!user) {
      return res
        .status(400)
...
router.get("/:id",  async (req, res) => {
    try {
      const user = await UserDUT.findById(req.params.id).select("-password");
      if (!user) {
        return res
          .status(400)
...
router.get("/getallusers", async (req, res) => {
    try {
      const users = await UserDUT.find({});
      if (!users) {
...
router.put("/updateuser/:id",  async (req, res) => {
    try {
      const userUpdate = await UserDUT.findOneAndUpdate(
        { _id: req.userId },
        req.body,
        {
          new: true,
